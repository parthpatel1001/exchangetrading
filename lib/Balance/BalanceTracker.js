...
let setBalanceToRedis = (balanceIn) => {
    ...
    RedisWrapper.getValue(exSpecificBalanceKey,(err,reply) => {
        if(err){
            console.log('error returned from redisWrapper.getValue for: exid: ',exid,err);
            throw err;
        }
        console.log('setBalToRedis', balanceIn);
        ...
        if(!reply) {
            RedisWrapper.setValue(exSpecificBalanceKey,persistableBalance);
        ...
        if(!balanceIn.compare(replyBalance)) {
            ...
            RedisWrapper.publishValue(balanceChannel, persistableBalance);
        }
    });
};
...
export class BalanceTracker {
    ...
    trackBalance(interval,exch){
        setInterval(() => {
            exch.getBalance((err,result) => {
                ...
                console.log('Track Balance Result', result.Persistable);
    ...
    retrieveBalance(exch,callback){
        ...
        RedisWrapper.getValue(exSpecificBalanceKey,(err,result) => {
            if(err) {
                console.log('error in redisWrapper.getValue for: exid: ',exid,err);
