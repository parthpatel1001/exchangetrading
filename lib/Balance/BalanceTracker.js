...
let setBalanceToRedis = (balanceIn) => {
    ...
    RedisWrapper.getValue(exSpecificBalanceKey,(reply) => {
        ...
        if(!balanceIn.compare(replyBalance)) {
            console.log('SetBalanceToRedis','the balance changed',exSpecificBalanceKey,persistableBalance);
 ...
 */
function setBalancePropertyToRedis(balanceIn,balanceProperty){
    ...
    RedisWrapper.setValue(exSpecificBalanceKey,balanceProperty,toSet);
}
