...
var Arbiter = function() {
    ...
    var threshold = config.get('TradeThresholds.COINBASE_BITSTAMP_ARB_THRESHOLD'),
        OrderGenerator;
    ...
    var getArbOppurtunities = function(orderBookTop,callback,noTradeCallback,invalidBookCallBack)
    {
        ...
        if(dir1.cmp(threshold) === 1)
        {
            ...
            if(amount.cmp(.01)===1) {
                ...
                return callback(
                    [book1.exchangeId,book2.exchangeId],
                    book1.ask.price,
                    book2.bid.price,
                    amount
                );
        ...
        if(dir2.cmp(threshold) === 1)
        {
            ...
            if(amount.cmp(.01)===1) {
                ...
                return callback(
                    [book2.exchangeId,book1.exchangeId],
                    book2.ask.price,
                    book1.bid.price,
                    amount
                );
    ...
    this.registerOrderGenerator = function(OrderGeneratorIn)
    {
        OrderGenerator = OrderGeneratorIn;
    };

    this.subscribeTo2ExchangeArbOppurtunities = function(bookTop) {
        return getArbOppurtunities(bookTop,OrderGenerator.generateOrder,OrderGenerator.noOrder,OrderGenerator.err);
    };
};
