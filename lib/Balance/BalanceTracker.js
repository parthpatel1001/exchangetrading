...
let setBalanceToRedis = (balanceIn) => {
    ...
    RedisWrapper.getValue(exSpecificBalanceKey,(reply) => {
        ...
        if(!balanceIn.compare(replyBalance)) {
            Notification.eventTriggered("TrackBalance Changed", { "New Balance": serialized }, "", NotificationLevels.LOW);
...
export class BalanceTracker {
    ...
    trackBalance(interval,exch){
        ...
        doTracking(); // Trigger the tracking when being setup so we don't have to wait until after the first interval to get data
    };
