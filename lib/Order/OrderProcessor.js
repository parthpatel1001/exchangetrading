var Notification = require('../Notification.js'), // TODO MOVE THIS TO A NAMESPACE/DOMAIN FOLDER
    BalanceTracker = require('../Balance/BalanceTracker.js'),
    async = require('async'),
...
var OrderProcessor = function(ExchangeManager) {
     ...
     * @returns {boolean}
     */
    this.makeSureEnoughBalance = function(balance,order,exchange){
        if(order.isBuyOrder()) {
     ...
     * @param exchange
     */
    this.adjustBalance = function(order,exchange) {
        var cost = order.getCost(exchange.getFee());
        if(order.isBuyOrder()) {
    ...
    this.processLinkedOrder = function(order1,order2) {
        ...
        async.parallel([
            ...
            async.parallel([
                function(callback) {
                    callback(null,self.makeSureEnoughBalance(balance1,order1,ex1));
                },
                function(callback) {
                    callback(null,self.makeSureEnoughBalance(balance2,order2,ex2));
                }
            ],function(err,results){
                if(err) {
                ...
                function(err,results){
                    ...
                    async.parallel([
                        function(){ self.adjustBalance(order1,ex1)},
                        function(){ self.adjustBalance(order2,ex2)},
                        function(){
